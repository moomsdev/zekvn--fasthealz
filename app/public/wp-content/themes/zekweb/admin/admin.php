...
function change_form_text($translation, $text)
{
    ...
    if ('Log In' == $text || 'Log in' == $text) {
        return 'Tiếp tục';
    }
    if ('Lost your password?' == $text) {
        return 'Quên mật khẩu?';
    }
    if ('&larr; Back to %s' == $text) {
    ...
    if ('A password will be e-mailed to you.' == $text) {
        return 'Mật khẩu sẽ được chuyển đến email của bạn.';
    }
    if ('Username or E-mail:' == $text) {
        return 'User hoặc email';
    }
    if ('Get New Password' == $text) {
        return 'Nhận mật khẩu mới';
    }
    if ('You are now logged out.' == $text) {
